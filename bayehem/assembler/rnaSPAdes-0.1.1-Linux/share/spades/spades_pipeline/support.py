...
def get_spades_binaries_info_message():
    return "You can obtain rnaSPAdes binaries in one of two ways:" +\
           "\n1. Download them from http://bioinf.spbau.ru/en/rnaspades" +\
           "\n2. Build source code with ./spades_compile.sh script"


def check_binaries(binary_dir, log):
    for binary in ["hammer", "ionhammer", "spades", "bwa-spades"]:
        binary_path = os.path.join(binary_dir, binary)
        if not os.path.isfile(binary_path):
            error("rnaSPAdes binaries not found: " + binary_path + "\n" + get_spades_binaries_info_message(), log)
