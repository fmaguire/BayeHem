...
#rna
draft_assembly = False
min_complete_transcript = None
# list of spades.py options
long_options = "12= threads= memory= tmp-dir= iterations= phred-offset= sc iontorrent rna "\
               ...
               "mismatch-correction mismatch-correction:false careful careful:false draft-assembly draft-assembly:false min-complete-transcript= "\
...
def usage(spades_version, show_hidden=False, dipspades=False):
    if not dipspades:
        sys.stderr.write("rnaSPAdes genome assembler v." + str(spades_version) + "\n")
    else:
    ...
    sys.stderr.write("RNA specific options:" + "\n")
    sys.stderr.write("--draft-assembly\t\t relatively fast assembly with lower quality" +"\n")
    sys.stderr.write("--min-complete-transcript\t remove low-covered (coverage less than 4) isolated transcript fragments with shorted than the specified value" +"\n")
    sys.stderr.write("\t\t\t\t[default: %s]\n" % MIN_COMLETE_TRANSCRIPT)
    sys.stderr.flush()
...
def set_default_values():
    ...
    global cov_cutoff
    global tmp_dir
    global min_complete_transcript
    ...
    if min_complete_transcript is None:
        min_complete_transcript = MIN_COMLETE_TRANSCRIPT
